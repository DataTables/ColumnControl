// Very similar and large parts are repeated from colVis, but not all identical
describe('columnControl - colVisDropdown', function () {
	dt.libs({
	...
	describe('Functionality', function () {
		...
		it('Is present', () => {
			expect(DataTable.ColumnControl.content.colVisDropdown).toBeDefined();
		});

		it('Create CC dropdown with colVisDropdown', () => {
	...
	describe('Option - text', function () {
		...
		it('Create nested CC dropdown with colVis (default text)', () => {
			table = new DataTable('#example', {
				columnControl: [['colVisDropdown']]
			});
		...
		it('Got default button text', () => {
			expect($('.dtcc-dropdown .dtcc-button-text').text()).toBe('Column visibility');
		});
		...
		it('Show nested - Title text is not changed', () => {
			$('.dtcc-dropdown .dtcc-button').trigger('click');
